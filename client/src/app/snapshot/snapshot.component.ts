...
export class SnapshotComponent implements OnInit {
  ...
  public lineChart1Data: ChartDataset[] = [];
  public lineChart1Labels: string[] = [];

  public lineChart5Data: ChartDataset[] = [];
  public lineChart5Labels: string[] = [];
  ...
  load() {
    ...
    if (self.route.snapshot.queryParamMap.has('instrument')) {
      service = () =>
        self.snapshotService.create(
          self.route.snapshot.queryParamMap.get('instrument')
        );
    } else if (self.route.snapshot.queryParamMap.has('action')) {
      service = () =>
        self.snapshotService.create(
          self.route.snapshot.queryParamMap.get('action')
        );
    } else if (self.route.snapshot.queryParamMap.has('decision')) {
      service = () =>
        self.snapshotService.confirm(
          self.route.snapshot.queryParamMap.get('id'),
          self.route.snapshot.queryParamMap.get('decision'),
          self.route.snapshot.queryParamMap.get('confirmed')
        );
    } else {
      service = () =>
        self.snapshotService.get(self.route.snapshot.queryParamMap.get('id'));
    }
  ...
  setRates(snapshot) {
    ...
    function GetData(rates): ChartDataset[] {
        ...
        return [
          {
            ...
            pointHoverBackgroundColor: 'black',
            data: rates.map(function (v, i) {
              if (i <= previousSnapshotIndex) return v.C;
              return null;
            }),
          },
          {
          ...
          },
          {
            ...
            pointBackgroundColor: 'rgba(151,187,205)',
            pointHoverBackgroundColor: 'black',
            data: rates.map(function (v, i) {
              if (i >= previousSnapshotIndex) return snapshot.PreviousBuyRate;
              return null;
            }),
          },
        ];
    ...
    yearBefore.setMonth(endDate.getMonth() - 12);
    const startDate = yymmdd(yearBefore);
    ...
    function sliceData(data: ChartDataset) {
      return {
        ...data,
    ...
    if (snapshot.PreviousTime == null) {
      self.lineChart1Data = [sliceData(self.lineChart5Data[0])];
    } else {
      self.lineChart1Data = [
  ...
  setDecision(decision) {
    ...
    self.snapshotService
      .decide({
        id: self.snapshot.ID,
        decision: decision,
        confirm: self.snapshot.ConfirmDecision,
