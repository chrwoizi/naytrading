...
export class SnapshotsComponent extends ListBase<Snapshot> implements OnInit {
  ...
  ngOnInit() {
    ...
    self.spinner.show();
    self.loading = true;
    self.snapshotService.list(instrument)
      .pipe(first())
      .subscribe(
        data => {
          self.loading = false;
  ...
  filterItem(value, query): { value: boolean } {
    if (query == "buy" || query == "wait" || query == "wait1yr" || query == "wait2mo" || query == "autowait" || query == "sell") {
      ...
      return { value: value.Instrument.InstrumentName.toLowerCase().indexOf(query.toLowerCase()) != -1 };
    }
  }
}