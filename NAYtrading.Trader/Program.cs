using PublicHoliday;
using NAYtrading.Trader.Models;
using System;
...
namespace NAYtrading.Trader
{
    class Program
    {
        ...
        private static void Run()
        {
            try
            {
                string naytradingUser = ConfigurationManager.AppSettings["NAYtradingUser"];
                if (string.IsNullOrEmpty(naytradingUser))
                {
                    Console.Write("NAYtrading User: ");
                    naytradingUser = Console.ReadLine();
                }

                string naytradingPassword = ConfigurationManager.AppSettings["NAYtradingPassword"];
                if (string.IsNullOrEmpty(naytradingPassword))
                {
                    Console.Write("NAYtrading Password: ");
                    naytradingPassword = ConsoleHelper.ReadPassword('*');
                }
                ...
                while (true)
                {
                    ...
                    try
                    {
                        var naytradingClient = new NAYtradingClient();

                        naytradingClient.Login(naytradingUser, naytradingPassword).Wait();
                        naytradingClient.Refresh().Wait();
                            ...
                            using (var db = new TradeDBContext())
                            {
                                ...
                                if (suggestions.Any())
                                {
                                    using (var controller = new OrderController(brokerUser, brokerPassword, ConfigurationManager.AppSettings["ShowBrowser"] != "true", new Flatex(), globalLogger, new FlatexTanProvider(tanFilePassword)))
                                    {
                                        ...
                                        {
                                            ProcessSuggestion(suggestion, ref availableFunds, minBuyOrderPrice, maxBuyOrderPrice, orderFee, db, controller, naytradingClient, globalLogger);
                                        }
                                    }
                                }
                                else
        ...
        private static void ProcessSuggestion(TradeSuggestion suggestion, ref decimal availableFunds, decimal minBuyOrderPrice, decimal maxBuyOrderPrice, decimal orderFee, TradeDBContext db, OrderController controller, NAYtradingClient naytradingClient, ILogger globalLogger)
        {
            ...
            try
            {
                try
                {
                    ...
                    if (action == TradingAction.Buy)
                    {
                        ...
                        naytradingClient.SetInstrumentWeight(suggestion.Isin ?? suggestion.Wkn, "Trader-bought", decimal.Parse(ConfigurationManager.AppSettings["BoughtInstrumentWeight"])).Wait();
                    }
                    else if (action == TradingAction.Sell)
                    {
                        ...
                        availableFunds -= orderFee;
                        naytradingClient.SetInstrumentWeight(suggestion.Isin ?? suggestion.Wkn, "Trader-bought", 0).Wait();
                    }
