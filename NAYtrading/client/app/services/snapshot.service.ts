...
import { ListSnapshotsResponse, GetSnapshotResponse, DecideSnapshotResponse } from '../models/snapshot';
...
export class SnapshotService {
    ...
    list(instrumentId) {
        if (typeof (instrumentId) !== 'undefined') {
            return this.http.get<ListSnapshotsResponse>('/api/snapshots/' + instrumentId);
        }
        else {
    ...
    confirm(id, decision, confirmed) {
        return this.http.get<GetSnapshotResponse>('/api/confirm/' + id + '/' + decision + '/' + confirmed);
    }
