...
using NAYtrading.Common.Net;
using NAYtrading.Trader.Models;
using NAYtrading.Common.Net.Models;
using System;
...
{
    public class NAYtradingClient
    {
        private HttpProvider httpProvider;

        public NAYtradingClient()
        {
        ...
        {
            Console.WriteLine("Signing in at naytrading...");
            var url = ConfigurationManager.AppSettings["NAYtradingAddress"] + "/signin";
        ...
        public async Task SetInstrumentWeight(string isinOrWkn, string type, decimal weight)
        {
            try
            {
                Console.WriteLine("Setting weight " + type + " of instrument " + isinOrWkn + " to " + weight + " at naytrading...");
                var url = string.Format(ConfigurationManager.AppSettings["NAYtradingAddress"] + "/api/weight/{0}/{1}/{2}", isinOrWkn, type, weight);
        ...
        public async Task Refresh()
        {
            ...
            var url = string.Format(ConfigurationManager.AppSettings["NAYtradingAddress"] + "/api/export/user/trades/{0:yyyyMMdd}", date);
